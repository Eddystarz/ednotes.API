...
module.exports = {
    Query: {
      depts: combineResolvers(isAuthenticated, isAdmin, async (_,__, ___) =>{
        try {
      ...
      dept: combineResolvers(isAuthenticated, async (_,{id}, ___) =>{
        try {
    ...
    Mutation: {
      createDept:combineResolvers(isAuthenticated, isAdmin, async (_,{input} ) =>{
        try {
